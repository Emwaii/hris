...
<body>
	...
	<?php
		$dat = "Interpreter_".date('d/m/Y')."_".substr(md5(time()), 0, 4).".xls";
	...
	<table border="1">
        ...
        rfreelance.id_lang = language_pair.id_lang where rfreelance.id_job = 2 and COALESCE(rfreelance.deleted,0) = 0");
        ?>
		...
		<tr>
			...
			<td><?php if($row->namal == "Others"){echo "  ".ucfirst($row->others);}else{echo "  ".ucfirst($row->namal);}?></td>
